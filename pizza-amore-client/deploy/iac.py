...
def step_build(ctx: Context) -> None:
    ...
    if not ctx.sh.dry_run:
        ...
        if not (dist / "index.html").exists():
            raise DeployError(f"{ctx.cfg.build.dist}/index.html não foi gerado pelo Vite")
...
DOCKERFILE_RUNTIME = """\
# Modo "artifact": o build do React/Vite acontece no seu PC e este estágio
...
# Modo "source": o build do React/Vite acontece DENTRO do
...
verify_paths    = ["/"]
verify_tries    = 3
verify_contains = "Pizza Amore"      # trecho obrigatório na home (vazio desliga)
...
def _common_parser() -> argparse.ArgumentParser:
    ...
    c.add_argument("-r", "--root", default=S, help="raiz do projeto React/Vite")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        parents=[common],
        prog="iac.py",
        description="Orquestrador de deploy React/Vite → k3s/RKE2/Rancher (sem CI/CD).",
